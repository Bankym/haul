...
function sanitizeActionName(action) {
    ...
    if (
        ...
        action.startsWith('.')      // hidden files
    ) {
        return null;
    }

    // Only allow alphanumeric, hyphens, underscores
    // This is a whitelist approach - safest for filenames
            ...
            if (typeof action == 'string') {
                ...
                if (!sanitizedAction) {
                    return res.status(400).json({
                        error: `Invalid action name: "${action}". Only alphanumeric characters, hyphens, and underscores are allowed.`
                    });
                }
